		...
		foreach (string asset in importedAssets) {
			...
			using (FileStream stream = File.Open (filePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite)) {
				...
				foreach(string sheetName in sheetNames) {
					...
					cell = row.GetCell(4); p.category = (int)(cell == null ? 0 : cell.NumericCellValue);
					cell = row.GetCell(6); p.actionID = (int)(cell == null ? 0 : cell.NumericCellValue);
					cell = row.GetCell(7); p.burnID = (int)(cell == null ? 0 : cell.NumericCellValue);
					cell = row.GetCell(8); p.lotID = (int)(cell == null ? 0 : cell.NumericCellValue);
					cell = row.GetCell(9); p.minValue = (int)(cell == null ? 0 : cell.NumericCellValue);
					cell = row.GetCell(10); p.maxValue = (int)(cell == null ? 0 : cell.NumericCellValue);
						s.list.Add (p);
					}
					data.sheets.Add(s);
				}
